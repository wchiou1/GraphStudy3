(function($P) {
	...
	$P.D3.SoupEntityLink = $P.defineClass(
		$P.D3.Element,
		function D3SoupEntityLink(config) {
			if (!(this instanceof D3SoupEntityLink)) {return new D3SoupEntityLink(config);}
			...
			self.mainSelection = self.selection.append('g')
				.selectAll('path').data(function(d, i) {
					var data = [];
					var graphs = self.view.activeGraphs(d);
					/*
					graphs.forEach(function(graph, i) {	
							data.push({
								crossOffsetSource: (i - (graphs.length * 0.5) + 0.5) * 3,
								crossOffsetTarget: (i - (graphs.length * 0.5) + 0.5) * 3,
								color: graph.color,
								source: d.source,
								target: d.target});
								});
			...
			return this;},
		{
			update: function(layout) {
				...
				self.mainSelection.attr('d', function(d, i) {
					...
					//console.log(self.source.id + ' --> ' + self.target.id);
	...
	$P.D3.SoupEntityLink.appender = $P.D3.Element.appender.bind(undefined, $P.D3.SoupEntityLink);
